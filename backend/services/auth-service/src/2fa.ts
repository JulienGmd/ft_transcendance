...
export function verifySMS2FACode(userId: number, input: string): boolean {
  const db = new Database('auth.db');
  const user = db.prepare('SELECT sms_2fa_code, sms_2fa_expires_at FROM users WHERE id = ?').get(userId) as { sms_2fa_code: string, sms_2fa_expires_at: number } | undefined;
  db.close();
...
export function verifyEmail2FACode(userId: number, input: string): boolean {
  const db = new Database('auth.db');
  const user = db.prepare('SELECT email_2fa_code, email_2fa_expires_at FROM users WHERE id = ?').get(userId) as { email_2fa_code: string, email_2fa_expires_at: number } | undefined;
  db.close();
